  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/server/login/inventorylogin",
        formData
      );
      const userData = response.data; // Assuming the response contains user data
      userData.role = "inventory"; // Add role information, assuming it's 'admin'
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="bg-blue-100 p-2 rounded-md mt-4 text-sm">
            ...
            <ul className="list-disc list-inside p-4 text-blue-900">
              ...
              <li>Generate reports related to inventory management</li>
            </ul>
          </div>
        </form>
      </div>
