  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3000/server/login/reportslogin",
        formData
      );
      const userData = response.data; // Assuming the response contains user data
      userData.role = "reports"; // Add role information, assuming it's 'admin'
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="bg-blue-100 p-2 rounded-md mt-4 text-sm">
            <p className="font-semibold text-blue-900">
              Login for Reports Generation
            </p>
            <ul className="list-disc list-inside p-4 text-blue-900">
              <li>Generate predefined and custom reports</li>
              <li>Access insights and analytics based on system data</li>
              <li>Customize report criteria and parameters</li>
              <li>Share reports with stakeholders for decision-making</li>
            </ul>
          </div>
        </form>
      </div>
