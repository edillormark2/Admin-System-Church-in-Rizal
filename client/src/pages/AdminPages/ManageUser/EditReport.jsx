...
const EditReport = () => {
  ...
  const breadcrumbLinks = [
    ...
    { to: "/admin/manage-user/report-user", label: "Reports User" },
    { to: "", label: "Edit User" }
  ];
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/server/users/userreport/${userID}`);
      setFormData({
        ...formData,
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:3000/server/users/userreport/update/${userID}`,
        updateData
      );
